...
//LINKDISTANCE
    force.linkDistance(function(link) {
        return link.connection === "week" ? height/10 : height/4;
    });

//Append a SVG to the body of the html page. Assign this SVG as an object to svg
    var svg = d3.select("body").append("svg").attr('style', "position: absolute; ")
        .attr("width", width)
        .attr("height", height)
...
//Read the data from the mis element
    d3.json("data2.json", function (error, graph) {
        ...
        var circle2 = svg.append("circle")
            .attr("cx", 800)
            .attr("cy", 350)
            .attr("r", 350).attr("fill","#e6fff2");

        var circle = svg.append("circle")
                                 .attr("cx", 800)
                                 .attr("cy", 325)
                                 .attr("r", 110).attr("fill","#E1939C");
